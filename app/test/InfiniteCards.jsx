...
const InfiniteCards = ({contents}) => {
  ...
  gsap.registerPlugin(ScrollTrigger);
    ...
    useGSAP(()=> {
      ...
      const spacing = 0.1,    // spacing of the cards (stagger)
        ...
        cards = gsap.utils.toArray('.FeedbackGallery_cards li'),
  ...
  return (
      <div class="FeedbackGallery border border-green-400 w-full h-[50vh] absolute top-[5rem] 
      left-[50%] translate-x-[-50%]">
        <ul class="FeedbackGallery_cards border border-red-500
        top-[0%] left-[50%] absolute translate-x-[-50%]">
          {contents.map((content, index)=> (
            <li key={index} className='absolute w-[70vw] max-w-[500px] h-[50vh] border
            top-0 left-0'>{content.name}</li>
          ))}
        </ul>
        <div class="actions absolute border bottom-[25px] left-[50%] translate-x-[-50%]">
          <button class="prev border">Prev</button>
          <button class="next border">Next</button>
        </div>
      </div>
  )
}
